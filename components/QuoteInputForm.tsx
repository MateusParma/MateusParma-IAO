...
export const QuoteInputForm: React.FC<QuoteInputFormProps> = ({ onSubmit, onScanImage, isLoading, currency, setCurrency }) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    // Validação mínima obrigatória para prosseguir
    if (description.trim() && city.trim() && clientName.trim()) {
      onSubmit(description, city, images, currency, clientName, clientAddress, clientContact, includeDescriptions);
    } else {
      alert('Por favor, preencha o Nome do Cliente, a Cidade e a Descrição do Serviço.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in max-w-5xl mx-auto">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
                ...
                <div className="space-y-4">
                    <div>
                        <label htmlFor="city" className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Cidade (Base de Preço)</label>
                        <input id="city" type="text" value={city} onChange={(e) => setCity(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary focus:border-primary transition outline-none font-medium text-gray-800" placeholder="Ex: Lisboa" required />
                    </div>
                    <div>
                        <label htmlFor="currency" className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Moeda</label>
                        <div className="relative">
                            <select id="currency" value={currency} onChange={(e) => setCurrency(e.target.value as Currency)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary focus:border-primary transition outline-none appearance-none font-medium text-gray-800 cursor-pointer">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300 h-full flex flex-col">
                 <div className="flex items-center justify-between mb-4 border-b pb-2 border-gray-100">
                    ...
                    <div>
                        ...
                        <button type="button" onClick={() => scanInputRef.current?.click()} className="flex items-center text-xs font-bold bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-3 py-2 rounded-lg hover:shadow-md transition transform hover:-translate-y-0.5">
                ...
                <div className="flex-grow space-y-6">
                    <div>
                        <label htmlFor="description" className="block text-sm font-semibold text-gray-700 mb-2">O que precisa ser feito?</label>
                        <textarea id="description" rows={6} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full p-4 text-base bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary focus:border-primary transition outline-none resize-none" placeholder="Ex: Preciso pintar a sala de 20m²..." required />
                        <div className="mt-2 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="flex items-start text-xs text-gray-500 bg-blue-50 p-3 rounded-lg text-blue-800 flex-grow">
                                ...
                                Dica: Seja específico com quantidades (m², unidades) e ações.
                            </div>
                            <label className="flex items-center cursor-pointer bg-gray-50 px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-100 transition select-none flex-shrink-0">
                                <div className="relative">
                                    <input type="checkbox" className="sr-only" checked={includeDescriptions} onChange={(e) => setIncludeDescriptions(e.target.checked)} />
                                ...
                                <span className="ml-3 text-sm font-bold text-gray-700">Detalhar Etapas</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="group relative flex justify-center px-6 pt-10 pb-10 border-2 border-gray-300 border-dashed rounded-xl hover:bg-gray-50 hover:border-primary transition cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                            <div className="space-y-2 text-center">
                                ...
                                <div className="text-sm text-gray-600"><span className="font-semibold text-primary">Clique para enviar</span> ou arraste imagens</div>
                                <p className="text-xs text-gray-400">JPG, PNG (Max 10MB)</p>
                            </div>
                        </div>
                        <input ref={fileInputRef} id="file-upload" name="file-upload" type="file" className="sr-only" multiple accept="image/*" capture="environment" onChange={handleImageChange} />
                        {imagePreviews.length > 0 && (
                            <div className="mt-4">
                                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Imagens ({imagePreviews.length})</h4>
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    <div key={index} className="relative w-20 h-20 rounded-lg overflow-hidden shadow-sm border border-gray-200 group">
                                        ...
                                        <button type="button" onClick={() => removeImage(index)} className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white font-bold">✕</button>
                                    </div>
                                    ))}
                                </div>
                            </div>
      ...
      <div className="pt-4">
        <button
            type="submit"
            disabled={isLoading || !description.trim() || !city.trim() || !clientName.trim()}
