...
import type { QuoteData, TechnicalReportData, WarrantyData, ReceiptData, Currency } from '../types';
import { 
...
// Removed DocumentTextIcon which was causing an error
import { TrashIcon, EyeIcon, PencilIcon, ClipboardDocumentIcon, ShieldCheckIcon } from './AppIcons';
...
export const HistoryPage: React.FC<HistoryPageProps> = ({ onEditQuote, onEditReport, onEditWarranty, onEditReceipt }) => {
  ...
  const loadAllData = async () => {
    setLoading(true);
    const [q, r, w, rec] = await Promise.all([
      fetchQuotes(),
      fetchReports(),
      fetchWarranties(),
      fetchReceipts()
